...
function pre_computed_sr_rates(n, l, m, alph, M; n_high=20, n_low=20, delt_a=0.001, cheby=true)
    ...
    if !cheby
        fn = joinpath(rate_sve_dir, "Imag_zero_$(state_str).dat")
    else
        fn = joinpath(rate_sve_dir, "Imag_zeroC_$(state_str).dat")
    end
    if isfile(fn)
        zerolist = readdlm(fn)
    ...
    if run_high
        ...
        if !cheby
            fn = joinpath(rate_sve_dir, "Imag_erg_pos_$(state_str).npz")
        else
            fn = joinpath(rate_sve_dir, "Imag_ergC_pos_$(state_str).npz")
        end
        if isfile(fn)
            file_in = npzread(fn)
    ...
    if run_low
        ...
        if !cheby
            fn = joinpath(rate_sve_dir, "Imag_erg_neg_$(state_str).npz")
        else
            fn = joinpath(rate_sve_dir, "Imag_ergC_neg_$(state_str).npz")
        end
        if isfile(fn)
            file_in = npzread(fn)
